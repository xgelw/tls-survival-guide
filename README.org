...
Another verification that might be executed by a TLS client is to verify that the certificates purpose (e.g. TLS server, TLS client) is properly set. Again, OpenSSL can help to verify this:
